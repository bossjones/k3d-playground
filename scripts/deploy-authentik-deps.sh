...
kubectl create namespace identity || true
helm repo add authentik-redis https://bjw-s.github.io/helm-charts || true
helm repo update
...
# kustomize build --enable-alpha-plugins --enable-exec apps/argocd/base/kube-system/external-secrets apps/argocd/base/identity/authentik-redis | kubectl apply --server-side -f -
# kustomize build --enable-alpha-plugins --enable-exec apps/argocd/base/kube-system/external-secrets apps/argocd/base/database/cloudnative-pg | kubectl apply --server-side -f -
